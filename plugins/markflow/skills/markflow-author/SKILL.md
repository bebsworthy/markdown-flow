...
- **forEach**: A thick edge `==>|each: KEY|` fans out dynamically — the engine spawns one token per array element. The body is a sub-graph (all nodes reachable via thick edges from the entry) that supports branching (`==>|pass|`/`==>|fail|`), conditional skips, retry loops (`==>|fail max:N|`), and parallel fan-out (multiple unlabeled `==>` edges). Each item routes independently within the body. A normal edge (`-->`) exits to the collector. Configure `maxConcurrency` (0=unlimited, 1=serial, N=sliding window) and `onItemError` in the source step's `foreach:` config block. See `references/routing-and-config.md`.
