...
- `output:ref` — `{ stepSeq, tokenId, nodeId, stream: "stdout" | "stderr", path }`. Emitted at `step:start`, not `step:complete`, so that a crash mid-step still leaves a log record pointing at the sidecar file (otherwise the file is an orphan with no event referencing it). `bytes` is deliberately *not* on the event — consumers stat the file at read time. `stepSeq` is the envelope `seq` of the corresponding `step:start` — it's the key that uniquely identifies a single step execution, including across loop iterations. Named `stepSeq` (not `seq`) to avoid collision with the envelope's own `seq` field. See "Step output" below.
